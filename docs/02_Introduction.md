...
1. Защита интересов пользователя: предоставить инструмент для быстрого (< 1 минуты) выявления юридических рисков в цифровых договорах (оферты, политики конфиденциальности) перед их принятием.

2. Снижение когнитивной нагрузки: избавить пользователя от необходимости читать многостраничные юридические тексты, выделяя только критически важные ("горячие") места.

3. Монетизация: создать востребованный B2C сервис с freemium-моделью подписки и потенциалом B2B API для образовательных платформ и маркетплейсов.

### 2.2 Границы применимости (Scope)

1. География: Российская Федерация (учет законодательства РФ: ГК РФ, 152-ФЗ, ЗоЗПП).

2. Типы документов (MVP): Пользовательские соглашения (User Agreement), Политики конфиденциальности (Privacy Policy), Оферты онлайн-школ и сервисов подписки.

3. Платформа: Telegram-бот (отсутствие необходимости устанавливать отдельное приложение).

4. Первоочерёдная доступность в дневное время суток - с 8 до 19 часов.

5. Явная декларация невозможности хранения документов, загружаемых пользователем.

6. Вопросы по выбору и способу запуска LLM (локальные, облачные) в рамках данной работы не обсуждаются -- требуется провести отдельную аналитику по способам монетизации решения.
...
| LLM (Large Language Model) | большая языковая модель (GPT-3.5, YandexGPT, GigaChat), используемая для анализа текста и генерации отчётов |
