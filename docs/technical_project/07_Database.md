...
#### Сбор требований
К реализации функциональных требований к векторному хранилищу можно отнести указанные ниже моменты.

1. Для решения цели в рамках выполнения требования [ФТ-08](https://github.com/trofimovelijah/red-flag-analysis/issues/29) предлагается использовать решение на базе технологии RAG. Для сложных задач семантического поиска принято использовать `BERT`-подобные модели модели эмбеддингов размерностью 768-1024.   
2. Модели эмбеддингов необходимо для продакшн-среды брать с платформы Hugging Face.   
3. В рамках MVP на первом плане стоит работа с российскими юридическими документами, следовательно эмбеддинги должны быть заточены на русский язык, либо быть мультиязычными.
4. Объём данных. Необходимо зафиксировать текущий объем данных и прогнозируемый рост. Это критично, поскольку объём данных определяет выбор алгоритма индексирования и архитектуру развертывания. При объёме более 100 млн векторов требуются специализированные гибридные стратегии индексирования.  
5. К паттернам запросов относится:  
   1. семантический поиск (KNN/ANN) \- обязателен в рамках MVP  
   2. гибридный поиск (вектор \+ фильтрация по метаданным)  
   3. пре/пост фильтрация  
   4. конкретные примеры запросов

К нефункциональным требованиям, касающимся векторных хранилищ, отнесём следующие:

1. RAG-приложения требуют задержки \< 100 мс для всей цепочки (поиск+генерация), что отражено в **МШ-2**.  
2. Пропускная способность Throughput \= 1000 QPS. Количество одновременных запросов в секунду определяет требования к вычислительным ресурсам.   
3. Полнота поиска Recall rate \= 95%.   
4. High Availability \= 99,5%.  
5. Для текущего объёма данных возможно использовать одну коллекцию.
